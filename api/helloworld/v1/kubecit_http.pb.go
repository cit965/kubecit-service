...
const OperationKubecitMyOrderList = "/helloworld.v1.Kubecit/MyOrderList"
...
func RegisterKubecitHTTPServer(s *http.Server, srv KubecitHTTPServer) {
	...
	r.GET("/api/my_order", _Kubecit_MyOrderList0_HTTP_Handler(srv))
...
func _Kubecit_MyOrderList0_HTTP_Handler(srv KubecitHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in MyOrderRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationKubecitMyOrderList)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.MyOrderList(ctx, req.(*MyOrderRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type KubecitHTTPClient interface {
	...
	MyOrderList(ctx context.Context, req *MyOrderRequest, opts ...http.CallOption) (rsp *OrderListReply, err error)
...
func (c *KubecitHTTPClientImpl) MyOrderList(ctx context.Context, in *MyOrderRequest, opts ...http.CallOption) (*OrderListReply, error) {
	var out OrderListReply
	pattern := "/api/my_order"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationKubecitMyOrderList))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
